...
public:
	this(Engine engine) {
		...
		cam.SetupProjection(fov, GetWindow.WindowSize.x, GetWindow.WindowSize.y, 1, 10);
		cam.Update();
	...
	override void Update(Engine engine, Event[] events) {
		double delta = 0;
		foreach(Event event; events) {
			if (auto e = cast(ResizeEvent)event) {
				GetWindow.WindowSize = e.WindowSize;
		...
		auto _ = (KeyCode k) => GetKeyboard.Check(k);
