module swift.log.log;

/*version (Posix)
	import swift.log.terminal : stdout, stderr, Color;
else
...
public:
	...
	void Verbose(string this_=__FUNCTION__, S...)(lazy string format, lazy S args) {
		Log(LogLevel.VERBOSE, /*getName!*/this_, format, args);
	}
	
	void Debug(string this_=__FUNCTION__, S...)(lazy string format, lazy S args) {
			Log(LogLevel.DEBUG, /*getName!*/this_, format, args);
	}
	
	void Info(string this_=__FUNCTION__, S...)(lazy string format, lazy S args) {
		Log(LogLevel.INFO, /*getName!*/this_, format, args);
	}

	void Warning(string this_=__FUNCTION__, S...)(lazy string format, lazy S args) {
		Log(LogLevel.WARNING, /*getName!*/this_, format, args);
	}

	void Error(string this_=__FUNCTION__, S...)(lazy string format, lazy S args) {
		Log(LogLevel.ERROR, /*getName!*/this_, format, args);
	}

	void Fatal(string this_=__FUNCTION__, S...)(lazy string format, lazy S args) {
		Log(LogLevel.FATAL, /*getName!*/this_, format, args);
	}

	void Critical(S...)(lazy string format, lazy S args) {
		import std.c.stdlib : exit;
		Log(LogLevel.SEVERE, /*getName!*/this_, format, args);
		exit(-1);
	}
